...
<ConfigProvider form={{ validateMessages }}>
  <Form />
</ConfigProvider>;`]],["h2","Form.Item"],["p","表单字段组件，用于数据双向绑定、校验、布局等。"],["table",["thead",["tr",["th","参数"],["th","说明"],["th","类型"],["th","默认值"],["th","版本"]]],["tbody",["tr",["td","colon"],["td","配合 ",["code","label"]," 属性使用，表示是否显示 ",["code","label"]," 后面的冒号"],["td","boolean"],["td","true"],["td"]],["tr",["td","dependencies"],["td","设置依赖字段，说明",["a",{title:null,href:"#dependencies"},"见下"]],["td",["a",{title:null,href:"#NamePath"},"NamePath"],"[]"],["td","-"],["td"]],["tr",["td","extra"],["td","额外的提示信息，和 ",["code","help"]," 类似，当需要错误信息和提示文案同时出现时，可以使用这个。"],["td","ReactNode"],["td","-"],["td"]],["tr",["td","getValueFromEvent"],["td","设置如何将 event 的值转换成字段值"],["td","(..args: any[]) => any"],["td","-"],["td"]],["tr",["td","getValueProps"],["td","为子元素添加额外的属性"],["td","(value: any) => any"],["td","-"],["td","4.2.0"]],["tr",["td","hasFeedback"],["td","配合 ",["code","validateStatus"]," 属性使用，展示校验状态图标，建议只配合 Input 组件使用"],["td","boolean"],["td","false"],["td"]],["tr",["td","help"],["td","提示信息，如不设置，则会根据校验规则自动生成"],["td","ReactNode"],["td","-"],["td"]],["tr",["td","htmlFor"],["td","设置子元素 label ",["code","htmlFor"]," 属性"],["td","string"],["td","-"],["td"]],["tr",["td","initialValue"],["td","设置子元素默认值，如果与 Form 的 ",["code","initialValues"]," 冲突则以 Form 为准"],["td","string"],["td","-"],["td","4.2.0"]],["tr",["td","noStyle"],["td","为 ",["code","true"]," 时不带样式，作为纯字段控件使用"],["td","boolean"],["td","false"],["td"]],["tr",["td","label"],["td",["code","label"]," 标签的文本"],["td","ReactNode"],["td","-"],["td"]],["tr",["td","labelAlign"],["td","标签文本对齐方式"],["td",["code","left"]," ","|"," ",["code","right"]],["td",["code","right"]],["td"]]]],["p","<<<<<<< HEAD | labelCol | ",["code","label"]," 标签布局，同 ",["code","<Col>"]," 组件，设置 ",["code","span"]," ",["code","offset"]," 值，如 ",["code","{span: 3, offset: 12}"]," 或 ",["code","sm: {span: 3, offset: 12}"],"。你可以通过 Form 的 ",["code","labelCol"]," 进行统一设置。当和 Form 同时设置时，以 Item 为准 | ",["a",{title:null,href:"/components/grid/#Col"},"object"]," | - | | | messageVariables | 默认验证字段的信息 | Record<string, string> | - | 4.7.0 | ======= | labelCol | ",["code","label"]," 标签布局，同 ",["code","<Col>"]," 组件，设置 ",["code","span"]," ",["code","offset"]," 值，如 ",["code","{span: 3, offset: 12}"]," 或 ",["code","sm: {span: 3, offset: 12}"],"。你可以通过 Form 的 ",["code","labelCol"]," 进行统一设置，不会作用于嵌套 Item。当和 Form 同时设置时，以 Item 为准 | ",["a",{title:null,href:"/components/grid/#Col"},"object"]," | - | |"],["blockquote",["blockquote",["blockquote",["blockquote",["blockquote",["blockquote",["blockquote",["p","origin/master | name | 字段名，支持数组 | ",["a",{title:null,href:"#NamePath"},"NamePath"]," | - | | | preserve | 当字段被删除时保留字段值 | boolean | true | 4.4.0 | | normalize | 组件获取值后进行转换，再放入 Form 中 | (value, prevValue, prevValues) => any | - | | | required | 必填样式设置。如不设置，则会根据校验规则自动生成 | boolean | false | | | rules | 校验规则，设置字段的校验逻辑。点击",["a",{title:null,href:"#components-form-demo-basic"},"此处"],"查看示例 | ",["a",{title:null,href:"#Rule"},"Rule"],"[] | - | | | shouldUpdate | 自定义字段更新逻辑，说明",["a",{title:null,href:"#shouldUpdate"},"见下"]," | boolean ","|"," (prevValue, curValue) => boolean | false | | | tooltip | 配置提示信息 | ReactNode ","|"," ",["a",{title:null,href:"/components/tooltip#API"},"TooltipProps & { icon: ReactNode }"]," | - | 4.7.0 | | trigger | 设置收集字段值变更的时机 | string | ",["code","onChange"]," | | | validateFirst | 当某一规则校验不通过时，是否停止剩下的规则的校验。设置 ",["code","parallel"]," 时会并行校验 | boolean ","|"," ",["code","parallel"]," | false | ",["code","parallel"],": 4.5.0 | | validateStatus | 校验状态，如不设置，则会根据校验规则自动生成，可选：'success' 'warning' 'error' 'validating' | string | - | | | validateTrigger | 设置字段校验的时机 | string ","|"," string[] | ",["code","onChange"]," | | | valuePropName | 子节点的值的属性，如 Switch 的是 'checked'。该属性为 ",["code","getValueProps"]," 的封装，自定义 ",["code","getValueProps"]," 后会失效 | string | ",["code","value"]," | | | wrapperCol | 需要为输入控件设置布局样式时，使用该属性，用法同 ",["code","labelCol"],"。你可以通过 Form 的 ",["code","wrapperCol"]," 进行统一设置，不会作用于嵌套 Item。当和 Form 同时设置时，以 Item 为准 | ",["a",{title:null,href:"/components/grid/#Col"},"object"]," | - | | | hidden | 是否隐藏字段（依然会收集和校验字段） | boolean | false | |"]]]]]]]],["p","被设置了 ",["code","name"]," 属性的 ",["code","Form.Item"]," 包装的控件，表单控件会自动添加 ",["code","value"],"（或 ",["code","valuePropName"]," 指定的其他属性） ",["code","onChange"],"（或 ",["code","trigger"]," 指定的其他属性），数据同步将被 Form 接管，这会导致以下结果："],["ol",["li",["p","你",["strong","不再需要也不应该"],"用 ",["code","onChange"]," 来做数据收集同步（你可以使用 Form 的 ",["code","onValuesChange"],"），但还是可以继续监听 ",["code","onChange"]," 事件。"]],["li",["p","你不能用控件的 ",["code","value"]," 或 ",["code","defaultValue"]," 等属性来设置表单域的值，默认值可以用 Form 里的 ",["code","initialValues"]," 来设置。注意 ",["code","initialValues"]," 不能被 ",["code","setState"]," 动态更新，你需要用 ",["code","setFieldsValue"]," 来更新。"]],["li",["p","你不应该用 ",["code","setState"],"，可以使用 ",["code","form.setFieldsValue"]," 来动态改变表单值。"]]],["h3","dependencies"],["p","当字段间存在依赖关系时使用。如果一个字段设置了 ",["code","dependencies"]," 属性。那么它所依赖的字段更新时，该字段将自动触发更新与校验。一种常见的场景，就是注册用户表单的“密码”与“确认密码”字段。“确认密码”校验依赖于“密码”字段，设置 ",["code","dependencies"]," 后，“密码”字段更新会重新触发“校验密码”的校验逻辑。你可以参考",["a",{title:null,href:"#components-form-demo-register"},"具体例子"],"。"],["p",["code","dependencies"]," 不应和 ",["code","shouldUpdate"]," 一起使用，因为这可能带来更新逻辑的混乱。"],["p","从 ",["code","4.5.0"]," 版本开始，",["code","dependencies"]," 支持使用 render props 类型 children 的 ",["code","Form.Item"],"。"],["h3","shouldUpdate"],["p","Form 通过增量更新方式，只更新被修改的字段相关组件以达到性能优化目的。大部分场景下，你只需要编写代码或者与 ",["a",{title:null,href:"#dependencies"},["code","dependencies"]]," 属性配合校验即可。而在某些特定场景，例如修改某个字段值后出现新的字段选项、或者纯粹希望表单任意变化都对某一个区域进行渲染。你可以通过 ",["code","shouldUpdate"]," 修改 Form.Item 的更新逻辑。"],["p","当 ",["code","shouldUpdate"]," 为 ",["code","true"]," 时，Form 的任意变化都会使该 Form.Item 重新渲染。这对于自定义渲染一些区域十分有帮助："],["pre",{lang:"jsx",highlighted:`<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>Form.Item</span> <span class="token attr-name">shouldUpdate</span><span class="token punctuation">></span></span>
...
</Form>`]],["h2","Form.List"],["p","为字段提供数组化管理。"],["table",["thead",["tr",["th","参数"],["th","说明"],["th","类型"],["th","默认值"],["th","版本"]]],["tbody",["tr",["td","name"],["td","字段名，支持数组"],["td",["a",{title:null,href:"#NamePath"},"NamePath"]],["td","-"],["td"]],["tr",["td","children"],["td","渲染函数"],["td","(fields: Field[], operation: { add, remove, move }) => React.ReactNode"],["td","-"],["td"]],["tr",["td","rules"],["td","校验规则，仅支持自定义规则。需要配合 ",["a",{title:null,href:"#Form.ErrorList"},"ErrorList"]," 一同使用。"],["td","{ validator, message }[]"],["td","-"],["td","4.7.0"]]]],["pre",{lang:"tsx",highlighted:`<span class="token operator">&lt;</span>Form<span class="token punctuation">.</span>List<span class="token operator">></span>
